...
class Assignment3:
    ...
    def trapes_area(self, baseA: float, baseB: float, dx: float) -> np.float32:
        return np.float32((baseA + baseB) * dx / 2)
        pass

    def integrate2(self, f: callable, a: float, b: float, n: int) -> np.float32:
        ypoints = [np.float32(0.0)] * n;
        dx = (abs(a - b) / n)
        for i in range(0, n):
            x = a + dx * i
            ypoints[i] = f(x)
        ...
        for i in range(1, n):
            areasum += self.trapes_area(ypoints[i - 1], ypoints[i], dx)
        return areasum
        pass
    ...
    def integrate(self, f: callable, a: float, b: float, n: int) -> np.float32:
        ...
        result = np.float32(self.integrate2(f, a, b, n))

        return result

    def areabetween2(self, f1: callable, f2: callable, n: int) -> np.float32:
        # assuming qustion 2 is correct
        array = Assignment2.intersections2(f1, f2, 1, 100, n)
        if len(array) < 2:
        ...
        for i in range(0, n):
            High =f1ypoints[i] - f2ypoints[i]
            # if (np.sign(base1) != np.sign(base2)):
            #     areasum += abs(base1) * dx/2 + abs(base2) * dx/2
    ...
    def areabetween(self, f1: callable, f2: callable) -> np.float32:
        ...
        # replace this line with your solution
        result = self.areabetween2(f1,f2,100)
